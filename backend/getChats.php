...
if ($stmt = $conn->prepare("SELECT c.*, u.fullname FROM chats AS c JOIN users AS u ON c.user_id =  u.user_id  WHERE (c.user_id = ? AND reciever = ?) OR (c.user_id = ? AND reciever = ?) ORDER BY c.msg_id ASC")) {

...
// Get the search data and remove the '#' symbol if present
$searchData = isset($_GET['searchdata']) ? $_GET['searchdata'] : '';

// Remove the '#' symbol from the search query to make it more flexible
$searchData = ltrim($searchData, '#');
...
$query = "SELECT p.*, i.image_path, i.status, u.user_id, u.username, u.profilePic,
    ...
    WHERE p.status = 'public' AND i.status = 1 
    AND (p.hashtag LIKE ? OR p.caption LIKE ?)   -- Added OR condition to search in caption
    ORDER BY p.createdAt DESC
";

// Prepare the query and bind parameters
$stmt = $conn->prepare($query);
$searchWithHash = "%" . $searchData . "%";  // For partial matching (with '%')
$stmt->bind_param("iss", $currentUserId, $searchWithHash, $searchWithHash);
$stmt->execute();
